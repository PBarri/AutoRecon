	...
	def manual(self, service, plugin_was_run):
		if self.get_global('username') and self.get_global('password'):
			service.add_manual_command('ldapsearch command (modify before running):', [
				'ldapsearch -x -D "' + self.get_global('username') + '" -w "' + self.get_global('password') + '" -H ldap://{address}:{port} -b "dc=example,dc=com" -s sub "(objectclass=*)" 2>&1 | tee > "{scandir}/{protocol}_{port}_ldap_all-entries.txt"'
			])
		else:
			service.add_manual_command('ldapsearch command (modify before running):', [
