...
class Top100UDPPortScan(PortScan):
	...
	async def run(self, target):
		...
		if os.getuid() == 0 or config['disable_sanity_checks']:
			if target.ports:
				if target.ports['udp']:
					process, stdout, stderr = await target.execute('nmap {nmap_extra} -sU -A --osscan-guess -p ' + target.ports['udp'] + ' -oA "{scandir}/_custom_ports_udp_nmap" {address}', blocking=False)
				else:
					return []
			else:
				process, stdout, stderr = await target.execute('nmap {nmap_extra} -sU -A --top-ports 100 -oA "{scandir}/_top_100_udp_nmap" {address}', blocking=False)
			services = []
			while True:
