...
class GuessPortScan(PortScan):
	...
	async def run(self, target):
		if target.ports:
			if target.ports['tcp']:
				process, stdout, stderr = await target.execute('nmap {nmap_extra} -A --osscan-guess --version-all -p ' + target.ports['tcp'] + ' -oA "{scandir}/_custom_ports_tcp_nmap" {address}', blocking=False)
			else:
				return []
		else:
			process, stdout, stderr = await target.execute('nmap {nmap_extra} -A --osscan-guess --version-all -p- -oA "{scandir}/_quick_tcp_nmap" {address}', blocking=False)
